...
**功能**: 在训练过程中定期保存纯模型权重文件，便于后续分析和使用

**特性**:
- ✅ **独立保存机制**: 与最佳模型和检查点保存完全独立
- ✅ **灵活配置**: 可自定义保存频率和保留数量
- ✅ **自动清理**: 可限制保留的权重文件数量，自动删除旧文件
- ✅ **多重备份**: 时间戳目录和latest目录双重保存

**配置选项**:
```yaml
save:
  # 定期权重保存配置
  enable_periodic_weights_save: true     # 是否启用定期权重保存
  weights_save_frequency: 10             # 每隔多少个epoch保存一次权重
  max_weights_to_keep: 5                 # 最多保留的权重文件数量（0=全部保留）
```

**保存策略**:
1. **保存时机**: 每隔 `weights_save_frequency` 个epoch保存一次
2. **文件命名**: `model_epoch_N.pth` 格式（N为epoch数）
3. **保存位置**: `weights/` 子目录中
4. **文件管理**: 超出 `max_weights_to_keep` 限制时自动删除最旧的文件

**文件结构示例**:
```
models/checkpoints/train_20240815_143022/
├── best_model.pth              # 最佳模型
├── checkpoint_epoch_50.pth     # 检查点文件
└── weights/                    # 🆕 定期权重保存目录
    ├── model_epoch_10.pth      # 第10轮权重
    ├── model_epoch_20.pth      # 第20轮权重
    ├── model_epoch_30.pth      # 第30轮权重
    ├── model_epoch_40.pth      # 第40轮权重
    └── model_epoch_50.pth      # 第50轮权重
```

**使用场景**:
- 📊 **性能分析**: 对比不同训练阶段的模型性能
- 🔍 **收敛分析**: 观察模型在训练过程中的变化
- 🎯 **最优选择**: 从多个保存点中选择最适合的模型
- 🔄 **实验重现**: 从特定训练阶段开始新的实验

**注意事项**:
- 权重文件仅包含模型参数，不含优化器状态
- 用于恢复训练请使用检查点文件而非权重文件
- 权重文件可直接用于推理和评估
...
- **配置备份**: 每次训练都会自动备份配置文件
- **🆕 权重保存策略**:
  - 根据训练轮数设置合适的权重保存频率（如总轮数的10-20%）
  - 长期训练建议保留更多权重文件，短期训练可减少保留数量
  - 定期清理不需要的权重文件以节省存储空间
