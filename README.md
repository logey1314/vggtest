...
save:
  ...
  save_frequency: 50                # 检查点保存频率
  
  # 🆕 定期权重保存配置
  enable_periodic_weights_save: true  # 启用定期权重保存
  weights_save_frequency: 10          # 每10个epoch保存一次权重
  max_weights_to_keep: 5              # 最多保留5个权重文件
...
Epoch 10/50 [Train]: 100%|██████| Loss: 0.4521, Acc: 83.12%
Epoch 10/50 [Val]:   100%|██████| Loss: 0.3984, Acc: 86.78%
💾 定期权重保存: model_epoch_10.pth
...
### 🆕 智能权重管理
- **定期权重保存**: 可配置的自动权重保存，独立于最佳模型保存
- **灵活保存策略**: 自定义保存频率，支持每N个epoch保存权重
- **自动空间管理**: 可限制保留的权重文件数量，自动清理旧文件
- **多重备份机制**: 时间戳目录和latest目录双重保存，确保数据安全
- **便于分析对比**: 保存的权重文件便于对比不同训练阶段的模型性能
